...
class GithubAuthenticationFilterTest {
    ...
    void authenticationFilterWithState(UserStub user) throws Exception {
        ...
        final String state = mockMvc.perform(MockMvcRequestBuilders.get("/oauth2/authorization/github").session(session))
                .andExpect(MockMvcResultMatchers.status().is3xxRedirection())
                .andReturn().getResponse().getHeader(HttpHeaders.LOCATION).split("&state=")[1];

        mockMvc.perform(MockMvcRequestBuilders.get(
                        "/login/oauth2/code/github?code=" + user.code + "&state=" + state
    ...
    private void stubForUser(UserStub user) throws JsonProcessingException {
        stubFor(get(urlEqualTo("/user")).withHeader(
